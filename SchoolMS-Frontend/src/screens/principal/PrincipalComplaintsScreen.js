...
import {fetchPrincipalComplaints, replyComplaintThunk} from '../../redux/slices/complaintSlice';
...
const PrincipalComplaintsScreen = ({navigation}) => {
  const insets = useSafeAreaInsets();
  const dispatch = useDispatch();
  const {principalList, loading} = useSelector(s => s.complaints);
  const [filter, setFilter] = useState('all');

  const load = useCallback(() => {
    dispatch(fetchPrincipalComplaints(filter === 'all' ? {} : {status: filter}));
  }, [dispatch, filter]);
  ...
  const updateStatus = item => {
    Alert.alert('Update status', `Set "${item.title}" to:`, [
      ...
      {text: 'Rejected',  onPress: () => dispatch(replyComplaintThunk({id: item.id, data: {status: 'rejected'}}))},
      ...
      <View style={styles.card}>
        <View style={styles.row}>
          ...
          <View style={[styles.badge, {backgroundColor: c.bg}]}>
            <Text style={{color: c.fg, fontSize: 10, fontWeight: '800', textTransform: 'uppercase'}}>{item.status}</Text>
          </View>
        </View>
        <Text style={styles.desc} numberOfLines={3}>{item.description}</Text>
        <View style={styles.meta}>
          <Text style={styles.metaItem}>👤 {item.submitter?.name || 'User'} ({item.submitter?.role})</Text>
          {item.tagged_role && <Text style={styles.metaItem}>📌 {item.tagged_role}</Text>}
          {item.complaint_type === 'teacher_to_parent' && (
            <Text style={[styles.metaItem, {color: '#0984E3'}]}>↪ to parent</Text>
          )}
        </View>
        <View style={styles.actions}>
          <TouchableOpacity onPress={() => updateStatus(item)} style={[styles.btn, {backgroundColor: '#EEEBFF'}]}>
...
const styles = StyleSheet.create({
  ...
  chip: {paddingHorizontal: 10, paddingVertical: 5, borderRadius: 14, borderWidth: 1, borderColor: '#E5E7EB'},
...
export default PrincipalComplaintsScreen;
