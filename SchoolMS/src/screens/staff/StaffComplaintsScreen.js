...
import {fetchStaffComplaints, replyComplaintThunk} from '../../redux/slices/complaintSlice';
...
const StaffComplaintsScreen = ({navigation}) => {
  const insets = useSafeAreaInsets();
  const dispatch = useDispatch();
  const {staffList, loading} = useSelector(s => s.complaints);
  const [filter, setFilter] = useState('all');

  const load = useCallback(() => {
    dispatch(fetchStaffComplaints(filter === 'all' ? {} : {status: filter}));
  }, [dispatch, filter]);
    ...
    return (
      <View style={styles.card}>
        ...
        <View style={styles.meta}>
          <Text style={styles.metaItem}>👤 {item.submitter?.name || 'User'}</Text>
